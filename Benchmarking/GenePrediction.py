...
class Impute:
    ...
    def __init__(self, RNA_path, Spatial_path, location_path, count_path = None, device = None, train_list = None, test_list = None, norm = None):
    ...
    def pool(self, need_tools):
        ...
        if 'LIGER' in need_tools:
            ...
            for k in range(len(self.train_list)):
                R[k] = os.popen('Rscript Liger.r ' + self.RNA_file + ' ' + self.Spatial_file + ' ' + self.train_list[k] + ' ' + self.test_list[k] + self.norm)
            Result_Liger = pd.concat([RLiger])
            Result_Liger.to_csv("./impute_output/result_LIGER.csv",header=1, index=1)

        if 'Seurat' in need_tools:
            RSeurat = {}
            for k in range(len(self.train_list)):
                RSeurat[k] = os.popen('Rscript Liger.r ' + self.RNA_file + ' ' + self.Spatial_file + ' ' + self.train_list[k] + ' ' + self.test_list[k] + self.norm)
            Result_Seurat = pd.concat([RSeurat])
            Result_Seurat.to_csv("./impute_output/result_Seurat.csv",header=1, index=1)
